...
public class TasksController {
	...
	public @ResponseBody Tasks createTask(@RequestBody String payload) {
		...
		try {
			...
			// If 'id' in JSON payload is not null, get me the true int value of 'id' or set
			// it to 0

			Long parentId = jsonObject.get("parentId") != null
					? Double.valueOf(jsonObject.get("parentId").toString()).longValue()
					: Long.valueOf(1);
			...
			date = dueDate!=null ? Date.valueOf(dueDate) : null;
			// If 'id' is greater than zero, we are updating an existing note
			added = tasksRepo.add(parentId, statusStr, taskTitleStr, date);
	...
	/**
	 * This is hacky, but when you delete a note, the list of tasks is returned
	 * whether it succeeds or fails Technically, frontend could check for length to
	 * identify/resolve an error in deletion (non-existing note) Orginally I was
	 * returning a string here with success or error - but thats not very REST-y
	 */
	@CrossOrigin(origins = "http://localhost:3000")
	...
	public List<Tasks> deleteItem(@PathVariable(value = "id") int itemId) {
		...
		try {
			...
			if (tasks.getParentId() == 5) {
				...
				tasksRepo.update(tasks.getId().intValue(), tasks.getParentId(), tasks.getStatus(), tasks.getTaskTitle(),
						tasks.getDueDate());
			}
